...
export class SetupBedBedGroupService {
  ...
  async findAll(): Promise<SetupBedBedGroup[]> {
    const bed_group = await this.connection.query('SELECT bed_group.name, floor.name AS floor_name, bed_group.description, bed_group.color, bed_group.is_active  FROM bed_group  JOIN floor ON bed_group.floor = floor.id');
    return bed_group ;
  }

  
  async findOne(id: string): Promise<SetupBedBedGroup | null> {
    const bed_group = await this.connection.query('SELECT bed_group.name, floor.name AS floor_name, bed_group.description, bed_group.color, bed_group.is_active  FROM bed_group  JOIN floor ON bed_group.floor = floor.id WHERE bed_group.id = ?', [id]);
